...
from flask import Flask, render_template, url_for, redirect, flash, session  # CH: added flash and session
from flask_sqlalchemy import SQLAlchemy  # pip install Flask-SQLAlchemy
from flask_login import UserMixin, login_user, LoginManager, login_required, logout_user, current_user # pip Flask-Login
from flask_wtf import FlaskForm # pip install Flask-WTF
from wtforms import StringField, PasswordField, SubmitField, SelectField, RadioField, TextAreaField
from wtforms.validators import InputRequired, Length, ValidationError, Email
from flask_bcrypt import Bcrypt #used to hash passwords pip install Flask-Bcrypt
import datetime
...
# CH: added some pip installs as those may not be obvious ...
...
# CH where are these validations actually called from?
    ...
    if form.validate_on_submit():
        ...
        if user: #if user is registered
            if bcrypt.check_password_hash(user.password, form.password.data): #checks password, if everything matches up redirects to dashboard
                ...
                # CH: save username in session dict
                # not sure if this is the proper way to do this but it works ...
                session["user"] = user.username 
                return redirect(url_for('dashboard'))
            else:
                print("Wrong username or password!") 
                # Consider adding a flash error message: https://flask.palletsprojects.com/en/2.1.x/patterns/flashing/
